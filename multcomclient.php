<?php
	...
	if(isset($_POST['selector'])){
		foreach ($_POST['selector'] as $id):
			mysqli_query($con,"DELETE FROM commercial_owners WHERE id='$id'");
		endforeach;
...
for($i=0; $i < $N; $i++)
{
$result = mysqli_query($con,"SELECT * FROM commercial_owners where id='$id[$i]'");
...
<body>
	...
	<center><form class="form-horizontal" action="multeditcomclient.php" method="post">
	...
	<div style="margin-top: 0px; margin-bottom: 0px;">
		...
		<label class="control-label" for="accoun" style="font-weight:bold; font-size:18px;">Account Name:</label>
		<div class="controls">
		<input name="id[]" type="hidden" value="<?php echo  $row['id'] ?>" />
			<input name="account[]" type="text" style="font-weight:bold;" value="<?php echo $row['account'] ?>" />
		</div>
		</div>
...
<a class="btn btn-primary" href="cclients.php" style="margin-bottom: 20px;">Back</a>
</form>
</center>
